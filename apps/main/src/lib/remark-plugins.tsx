...
export function replaceLinks(options: { href: string } & WithChildren) {
  ...
  return options.href.startsWith("/") || options.href === "" ? (
    <Link href={options.href} className="cursor-pointer">{options.children}</Link>
  ) : (
    <Link href={options.href} target="_blank" rel="noopener noreferrer">{options.children} ↗</Link>
  );
}
