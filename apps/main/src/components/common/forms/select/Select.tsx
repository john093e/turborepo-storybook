    ...
    return (
        ...
        >
            <div className="relative w-full" ref={ref}>
                <div tabIndex={0} aria-expanded={open} onKeyDown={onPressEnterOrSpace} onClick={toggle} className={`flex text-sm rounded-lg shadow-sm bg-gray-50 border border-gray-300 text-gray-500 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 focus:outline-none dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500${isDisabled ? ' bg-gray-200' : ' bg-white hover:border-gray-400 focus:ring-2 focus:ring-blue-500'}`}>
                                ...
                                {Array.isArray(value) && (
                                    value.map((item, index) => (
                                        <div className={`bg-gray-200 border rounded-sm flex space-x-1${isDisabled ? ' border-gray-500 px-1' : ' pl-1'}`} key={index}>
                                            <p className="text-gray-800 truncate cursor-default select-none">{item.label}</p>
                                            {!isDisabled && (
                ...
                {(open && !isDisabled) && (
                    <div tabIndex={-1} className="absolute z-10 w-full bg-white shadow-lg border rounded-none py-1 mt-1.5 text-sm text-gray-700 dark:bg-gray-700 dark:border-none">
                        {isSearchable && (
                            <SearchInput
                                value={inputValue}
