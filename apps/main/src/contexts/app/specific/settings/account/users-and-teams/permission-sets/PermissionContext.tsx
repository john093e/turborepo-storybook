...
export type PermissionContextType = {
  ...
  handlePermissionSet: (access: string, category: string, value: number | boolean, type?: string) => void;
...
export function PermissionProvider({ data, id, userId, finishProcess, children }: Props) {  
  const [saving, setSaving] = useState<boolean>(false);

  const [permissionSetsDataCRM, setPermissionSetsDataCRM] = useState<PermissionSetsData["CRM"]>(data ? {
    "Contacts": {
  ...
  const [permissionSetsDataMarketing, setPermissionSetsDataMarketing] = useState<PermissionSetsData["Marketing"]>(data ? {
    "Lists": {
  ...
  const [permissionSetsDataSales, setPermissionSetsDataSales] = useState<PermissionSetsData["Sales"]>(data ? {
    ...
    "Create scheduling pages for others": data.sales_create_scheduling_pages_for_others,
    ...
    "Manage payments and subscriptions": data.sales_manage_payments_and_subscriptions
  } : settingsContextDefaultValues.permissionSetsDataSales);

  const [permissionSetsDataService, setPermissionSetsDataService] = useState<PermissionSetsData["Service"]>(data ? {
    "Service Access": data.service_service_access,
    "Templates": data.service_templates,
    "Create scheduling pages for others": data.service_create_scheduling_pages_for_others
  } : settingsContextDefaultValues.permissionSetsDataService);

  const [permissionSetsDataReports, setPermissionSetsDataReports] = useState<PermissionSetsData["Reports"]>(data ? {
  ...
  const [permissionSetsDataAccount, setPermissionSetsDataAccount] = useState<PermissionSetsData["Account"]>( data ? {
    ...
    "Modify billing and change name on contract": data.account_modify_billing_and_change_name_on_contract,
    "Add and edit developer apps and test accounts": data.account_add_and_edit_developer_apps_and_test_accounts,
  ...
  const handlePermissionSet = (access: string, category: string, value: number | boolean, type?: string) => {
    let newData;
    const permSet: undefined | PermissionSetsData["CRM"] | PermissionSetsData["Marketing"] | PermissionSetsData["Sales"] | PermissionSetsData["Service"] | PermissionSetsData["Reports"] | PermissionSetsData["Account"] =
      access === "CRM" ? { ...permissionSetsDataCRM } : access === "Marketing" ? { ...permissionSetsDataMarketing } : access === "Sales" ? { ...permissionSetsDataSales } : access === "Service" ? { ...permissionSetsDataService } : access === "Reports" ? { ...permissionSetsDataReports } : access === "Account" ? { ...permissionSetsDataAccount } : undefined;
    // check if we got data 
    if (permSet) {
      // check if it's a nested permission
      if (type && (type === "Create" || type === "Delete" || type === "Edit" || type === "Publish" || type === "View")) {
        ...
        if (Object.keys(permSetChild).length > 1) {
          ...
          if (type === "Create") {
            ...
            let toSave = { ...permSet, [category as keyof typeof permSet]: newData } as keyof typeof permSet;
                ...
                : access === "Sales"
                  ...
                  : access === "Service"
                    ...
                    : access === "Reports"
                      ...
                      : access === "Account"
                        ? setPermissionSetsDataAccount(toSave) : null;
        ...
        // get the nested permission
        newData = { ...permSet, [category as keyof typeof permSet]: value } as keyof typeof permSet;
        access === "CRM"
            ...
            : access === "Sales"
              ...
              : access === "Service"
                ...
                : access === "Reports"
                  ...
                  : access === "Account"
                    ? setPermissionSetsDataAccount(newData) : null;
      }
    } else {
      //do nothing
    }
  ...
  const [permissionDrawerError, setPermissionDrawerError] = useState<string | null>("");

  //Set As Super Admin
  const [setAsSuperAdmin, setSetAsSuperAdmin] = useState<boolean>(data ? data.super_admin : false);

  //Set As Super Admin with Sales Pro
  const [setAsSuperAdminWithSalesPro, setSetAsSuperAdminWithSalesPro] = useState<boolean>(data ? data.sales_sales_professional : false);

  //Set Permission Name
  const [permissionName, setPermissionName] = useState<string>(data ? data.name : "");

  //Save Permission
  const handleSaving = async () => {
    setSaving(true);
    if (permissionName.length > 0 && /^[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð]+(([',. -][a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ])?[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð]*)*$/.test(permissionName)) {
      if (
        permissionSetsDataCRM &&
        permissionSetsDataMarketing &&
        permissionSetsDataSales &&
        ...
        try {
          if (id) {
            const response = await fetch(
              "/api/settings/account/users-and-teams/permission-sets-drawer",
              {
